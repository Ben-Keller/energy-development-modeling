...
RUN_DIR_PATTERN = re.compile(r"^[a-f0-9]{8}$")
DEVELOPMENT_MODEL_DEFAULTS: Dict[str, Any] = {
    "surrogate": {
        "fallback_capex_share": 0.35,
        "fallback_opex_share": 0.65,
        "policy_multiplier": {
            "renewables_capex_sensitivity": 0.30,
            "fossil_price_sensitivity": 0.20,
            "carbon_price_per_100usd_sensitivity": 0.15,
            "demand_sensitivity": 0.20,
            "minimum": 0.35,
            "maximum": 2.5,
        },
        "totals": {
            "jobs_direct_capex_factor": 1.15,
            "jobs_direct_opex_factor": 0.40,
            "jobs_total_multiplier": 1.75,
            "gva_capex_factor": 0.30,
            "gva_opex_factor": 0.20,
            "household_income_share": 0.60,
        },
        "uncertainty_relative_bounds": {
            "jobs_direct": 0.20,
            "jobs_total": 0.20,
            "gva_total_musd": 0.20,
            "household_income_proxy_musd": 0.20,
        },
    },
    "mario": {
        "uncertainty_relative_bounds": {
...
def _create_run_dir(runs_dir: Path) -> Tuple[str, Path]:
...
def build_environment_setup_report(
    ...
    if development_model_path.exists():
        try:
            ...
            surrogate_present = "surrogate" in cfg
            ...
            _record(
                ...
                (
                    ...
                    f"Sections: surrogate={surrogate_present}, mario={mario_present}."
                ),
        ...
        _record(
            "ok",
            "development_model_config",
            "Optional development_model.csv not found; built-in default coefficients are active.",
            label="Development Model Controls Loaded",
            category="controls",
        )
    ...
    if development_engine in {"mario", "auto"}:
        if mario_health.get("ok"):
            ...
            message = f"MARIO required inputs missing: {missing}"
            if development_engine == "mario":
                _record("error", "mario_inputs", message, label="MARIO Inputs Loaded", category="mario")
            else:
                _record(
                    "warn",
                    "mario_inputs",
                    message + " (auto mode can fallback to surrogate).",
                    label="MARIO Inputs Loaded",
                    category="mario",
                )
    ...
    if settings.mario_db_path:
        ...
        if db_path.exists():
            ...
            if development_engine == "mario":
                ...
                _record("warn", "mario_db_path", msg, label="MARIO DB Path", category="mario")
...
def _load_development_model_config(config_dir: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.exception("Failed loading development model config at %s", path)
        return config
    for row in rows:
...
def _evaluate_mario_mapping_quality(config_dir: Path, tech_library: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    out["fallback_mapping_share"] = max(0.0, 1.0 - coverage)
    out["fallback_mapping_count"] = len(missing_techs)
...
def _match_rows_by_tech(df: pd.DataFrame, technology: str, tech_group: str) -> pd.DataFrame:
    ...
    wildcard = work[
        work["calliope_tech"].map(
            lambda p: _is_wildcard_pattern(str(p)) and fnmatch.fnmatchcase(tech, str(p))
        )
    ]
    ...
    generic = work[work["calliope_tech"].map(lambda x: _is_generic_token(str(x)))]
...
def _match_split_rows(
    ...
    if opex_type and "opex_type" in work.columns:
        ...
        if exact_type.empty:
            fallback_type = work[work["opex_type"].map(lambda x: _is_generic_token(str(x)) or _norm_key(x) == "mixed")]
            work = fallback_type if not fallback_type.empty else work
        else:
            work = exact_type
    if work.empty:
        return work
...
def _dominant_sector(df: pd.DataFrame) -> str:
    if df.empty or "mario_sector" not in df.columns:
        return ""
    work = df.copy()
    work["mario_sector"] = work["mario_sector"].fillna("").astype(str).str.strip()
    work = work[work["mario_sector"] != ""]
    if work.empty:
        return ""
    if "share" in work.columns:
        work["share"] = pd.to_numeric(work["share"], errors="coerce").fillna(0.0).clip(lower=0.0)
        agg = work.groupby("mario_sector", as_index=False)["share"].sum()
        if agg.empty:
            return ""
        agg = agg.sort_values("share", ascending=False)
        return str(agg.iloc[0]["mario_sector"]).strip()
    agg = work.groupby("mario_sector", as_index=False).size().sort_values("size", ascending=False)
    if agg.empty:
        return ""
    return str(agg.iloc[0]["mario_sector"]).strip()


def _infer_fallback_sectors(
    tech_map: pd.DataFrame,
    capex_split: pd.DataFrame,
    opex_split: pd.DataFrame,
) -> Dict[str, str]:
    ...
    out["capex"] = _dominant_sector(capex_split)
    out["opex"] = _dominant_sector(opex_split[opex_split.get("opex_type", "").astype(str).str.lower() == "om"]) if "opex_type" in opex_split.columns else ""
    out["fuel"] = _dominant_sector(opex_split[opex_split.get("opex_type", "").astype(str).str.lower() == "fuel"]) if "opex_type" in opex_split.columns else ""

    if (not out["capex"]) and (not tech_map.empty):
        capex_map = tech_map.copy()
        if "shock_channel" in capex_map.columns:
            capex_map = capex_map[capex_map["shock_channel"].astype(str).str.strip().str.lower() == "capex"]
        out["capex"] = _dominant_sector(capex_map)
    if (not out["opex"]) and (not tech_map.empty):
        opex_map = tech_map.copy()
        if "shock_channel" in opex_map.columns:
            opex_map = opex_map[opex_map["shock_channel"].astype(str).str.strip().str.lower() != "capex"]
        out["opex"] = _dominant_sector(opex_map)
    if not out["fuel"]:
        out["fuel"] = out["opex"]

    out["capex"] = out["capex"] or "Construction"
    out["opex"] = out["opex"] or "Electricity_and_heat"
    out["fuel"] = out["fuel"] or out["opex"]
    return out


def _build_region_weight_rows(
    summary_diagnostics: Dict[str, Any],
    pool_to_region: Dict[str, str],
) -> List[Dict[str, Any]]:
    ...
    for row in pool_rows:
        pool = str(row.get("pool", "")).strip() or "UNKNOWN"
        region = str(pool_to_region.get(pool, "")).strip() or _default_region_for_pool(pool)
        ...
        if score <= 0:
            continue
        rows.append({"region": region, "location": pool, "score": float(score)})
        score_total += float(score)

    if rows and score_total > 0:
        for row in rows:
            row["share"] = row["score"] / score_total
        return rows

    known_regions = sorted({str(v).strip() for v in pool_to_region.values() if str(v).strip()})
    fallback_region = known_regions[0] if known_regions else "UNKNOWN"
    return [{"region": fallback_region, "location": "ALL", "score": 1.0, "share": 1.0}]


def _extract_component_totals_for_fallback(
    summary: Dict[str, Any],
    summary_diagnostics: Dict[str, Any],
) -> Tuple[Dict[str, float], str]:
    totals_usd = {
        "investment": _sum_component_cost(summary_diagnostics, "investment"),
        "fixed_om": _sum_component_cost(summary_diagnostics, "fixed_om"),
        "variable_prod": _sum_component_cost(summary_diagnostics, "variable_prod"),
        "variable_con": _sum_component_cost(summary_diagnostics, "variable_con"),
    }
    if sum(max(v, 0.0) for v in totals_usd.values()) > 0:
        return totals_usd, "summary_diagnostics.cost_decomposition"

    class_rows = ((summary_diagnostics.get("cost_decomposition") or {}).get("class_totals") or {}).get("records") or []
    class_monetary = 0.0
    for rec in class_rows:
        ...
        if str(rec.get("costs", "")).strip().lower() != "monetary":
            continue
        class_monetary += max(_safe_float(rec.get("value"), 0.0), 0.0)
    if class_monetary > 0:
        return {
            "investment": 0.0,
            "fixed_om": 0.0,
            "variable_prod": class_monetary,
            "variable_con": 0.0,
        }, "summary_diagnostics.cost_decomposition.class_totals"

    system_monetary = _sum_cost_class(summary, "monetary")
    if system_monetary > 0:
        return {
            "investment": 0.0,
            "fixed_om": 0.0,
            "variable_prod": system_monetary,
            "variable_con": 0.0,
        }, "summary.system_cost"

    return totals_usd, "none"


def _build_fallback_exchange_from_summary(
    run_id: str,
    scenario: str,
    year: int,
    summary: Dict[str, Any],
    ...
    warnings: List[str] = []
    totals_usd, value_source = _extract_component_totals_for_fallback(summary, summary_diagnostics)
    totals_musd = {k: max(_safe_float(v, 0.0), 0.0) / 1_000_000.0 for k, v in totals_usd.items()}
    total_musd = sum(totals_musd.values())
    if total_musd <= 0:
        return pd.DataFrame(), pd.DataFrame(), {"used": False, "reason": "no_positive_monetary_costs"}, warnings

    sectors = _infer_fallback_sectors(tech_map=tech_map, capex_split=capex_split, opex_split=opex_split)
    region_rows = _build_region_weight_rows(summary_diagnostics=summary_diagnostics, pool_to_region=pool_to_region)

    inv_records: List[Dict[str, Any]] = []
    op_records: List[Dict[str, Any]] = []

    for region_row in region_rows:
        region = str(region_row.get("region", "")).strip() or "UNKNOWN"
        location = str(region_row.get("location", "")).strip() or "ALL"
        share = max(_safe_float(region_row.get("share"), 0.0), 0.0)
        if share <= 0:
            continue

        inv_shock = totals_musd["investment"] * share
        if inv_shock > 0:
            inv_records.append(
                {
                    "run_id": run_id,
                    ...
                    "region": region,
                    "location": location,
                    "technology": "fallback_investment",
                    "mario_sector": sectors["capex"],
                    "shock_channel": "capex",
                    "tech_group": "Other",
                    "shock_value_musd": inv_shock,
                }
            )

        for component, channel_key in (("fixed_om", "opex"), ("variable_prod", "opex"), ("variable_con", "fuel")):
            shock = totals_musd[component] * share
            if shock <= 0:
                continue
            op_records.append(
                {
                    ...
                    "region": region,
                    "location": location,
                    "technology": f"fallback_{component}",
                    "mario_sector": sectors[channel_key],
                    "shock_channel": "fuel" if component == "variable_con" else "opex",
                    "tech_group": "Other",
    ...
    if not inv_df.empty:
        inv_df = inv_df.groupby(
            ["run_id", "scenario", "year", "region", "location", "technology", "mario_sector", "shock_channel", "tech_group"],
            as_index=False,
        )["shock_value_musd"].sum()
    if not op_df.empty:
        op_df = op_df.groupby(
    ...
    warnings.append(
        "Exchange shocks fell back to summary-based allocation because tech-level Calliope monetary component rows were unavailable."
    )
    metadata = {
        "used": True,
        "value_source": value_source,
        "region_rows": int(len(region_rows)),
        ...
        "component_totals_musd": totals_musd,
    }
    return inv_df, op_df, metadata, warnings
...
def _write_exchange_files_for_mario(
    ...
    summary_diagnostics: Dict[str, Any],
    summary: Dict[str, Any],
) -> Tuple[Path, Dict[str, Any], Dict[str, str], List[str]]:
    warnings: List[str] = []
    ...
    if base_rows.empty:
        warnings.append("MARIO exchange builder found no tech-level monetary component rows; using summary-based fallback.")
    else:
    ...
    fallback_meta: Dict[str, Any] = {"used": False, "reason": ""}
    if inv_df.empty and op_df.empty:
        ...
        if not fb_inv.empty or not fb_op.empty:
            inv_df = fb_inv
            op_df = fb_op
        else:
            warnings.append("MARIO exchange shocks remain empty after fallback; runtime will emit zero development impacts.")
    ...
    activity_path = exchange_dir / "calliope_component_activity.csv"
    if base_rows.empty:
    ...
    inv_path = exchange_dir / "investment_shocks.csv"
    op_path = exchange_dir / "operating_shocks.csv"
    if inv_df.empty:
        pd.DataFrame(
            columns=[
                "run_id",
    ...
    prices_df.to_csv(exchange_dir / "prices_and_taxes.csv", index=False)
    ...
    metadata = {
        ...
        "fallback_exchange": fallback_meta,
    ...
    artifacts = {
        "energy_service_balance_csv": f"/api/run/{run_id}/download/exchange/energy_service_balance.csv",
        "calliope_component_activity_csv": f"/api/run/{run_id}/download/exchange/calliope_component_activity.csv",
        "investment_shocks_csv": f"/api/run/{run_id}/download/exchange/investment_shocks.csv",
        "operating_shocks_csv": f"/api/run/{run_id}/download/exchange/operating_shocks.csv",
        "prices_and_taxes_csv": f"/api/run/{run_id}/download/exchange/prices_and_taxes.csv",
    }
    shock_meta = {
        "investment_rows": int(len(inv_df)),
        "operating_rows": int(len(op_df)),
        ...
        "investment_total_musd": float(inv_df["shock_value_musd"].sum()) if not inv_df.empty else 0.0,
        "operating_total_musd": float(op_df["shock_value_musd"].sum()) if not op_df.empty else 0.0,
        "fallback_exchange_used": bool(fallback_meta.get("used", False)),
        "fallback_exchange_source": str(fallback_meta.get("value_source", fallback_meta.get("reason", ""))),
        "base_component_rows": int(len(base_rows)),
        "source_variable_by_component": source_by_component,
    }
    return exchange_dir, shock_meta, artifacts, warnings
...
def _build_surrogate_development_outputs(
    ...
    surrogate_cfg = development_model_config.get("surrogate") or {}
    summary_diagnostics = summary.get("summary_diagnostics") or {}
    monetary_cost = max(_sum_cost_class(summary, "monetary"), 0.0)

    capex_effect_musd = _sum_component_cost(summary_diagnostics, "investment")
    opex_effect_musd = (
        _sum_component_cost(summary_diagnostics, "fixed_om")
        + _sum_component_cost(summary_diagnostics, "variable_prod")
        + _sum_component_cost(summary_diagnostics, "variable_con")
    )

    fallback_capex_share = max(_config_float(surrogate_cfg, ["fallback_capex_share"], 0.35), 0.0)
    fallback_opex_share = max(_config_float(surrogate_cfg, ["fallback_opex_share"], 0.65), 0.0)
    share_total = fallback_capex_share + fallback_opex_share
    if share_total <= 0:
        fallback_capex_share, fallback_opex_share = 0.35, 0.65
    else:
        fallback_capex_share /= share_total
        fallback_opex_share /= share_total

    if capex_effect_musd <= 0 and opex_effect_musd <= 0:
        capex_effect_musd = monetary_cost * fallback_capex_share
        opex_effect_musd = monetary_cost * fallback_opex_share
        warnings.append(
            "Development proxy used fallback CAPEX/OPEX split because cost decomposition was unavailable."
        )

    policy_cfg = surrogate_cfg.get("policy_multiplier") or {}
    lever_values = req.levers.model_dump()
    policy_multiplier = (
        1.0
        + _config_float(policy_cfg, ["renewables_capex_sensitivity"], 0.30)
        * (1.0 - _safe_float(lever_values.get("renewables_capex_multiplier"), 1.0))
        + _config_float(policy_cfg, ["fossil_price_sensitivity"], 0.20)
        * (_safe_float(lever_values.get("fossil_fuel_price_multiplier"), 1.0) - 1.0)
        + _config_float(policy_cfg, ["carbon_price_per_100usd_sensitivity"], 0.15)
        * (_safe_float(lever_values.get("carbon_price_usd_per_tco2"), 0.0) / 100.0)
        - _config_float(policy_cfg, ["demand_sensitivity"], 0.20)
        * (_safe_float(lever_values.get("demand_multiplier"), 1.0) - 1.0)
    )
    policy_multiplier = max(
        _config_float(policy_cfg, ["minimum"], 0.35),
        min(_config_float(policy_cfg, ["maximum"], 2.5), policy_multiplier),
    )

    totals_cfg = surrogate_cfg.get("totals") or {}
    jobs_direct_capex_factor = _config_float(totals_cfg, ["jobs_direct_capex_factor"], 1.15)
    jobs_direct_opex_factor = _config_float(totals_cfg, ["jobs_direct_opex_factor"], 0.40)
    jobs_total_multiplier = _config_float(totals_cfg, ["jobs_total_multiplier"], 1.75)
    gva_capex_factor = _config_float(totals_cfg, ["gva_capex_factor"], 0.30)
    gva_opex_factor = _config_float(totals_cfg, ["gva_opex_factor"], 0.20)
    household_income_share = _config_float(totals_cfg, ["household_income_share"], 0.60)

    jobs_direct = max(
        (capex_effect_musd * jobs_direct_capex_factor + opex_effect_musd * jobs_direct_opex_factor)
        * policy_multiplier,
        0.0,
    )
    jobs_total = max(jobs_direct * jobs_total_multiplier, 0.0)
    gva_total_musd = max((capex_effect_musd * gva_capex_factor + opex_effect_musd * gva_opex_factor) * policy_multiplier, 0.0)
    household_income_proxy_musd = max(gva_total_musd * household_income_share, 0.0)

    uncertainty_cfg = surrogate_cfg.get("uncertainty_relative_bounds") or {}
    jobs_direct_low, jobs_direct_high = _relative_bounds(
        jobs_direct,
        _config_float(uncertainty_cfg, ["jobs_direct"], 0.20),
    )
    jobs_total_low, jobs_total_high = _relative_bounds(
        jobs_total,
        _config_float(uncertainty_cfg, ["jobs_total"], 0.20),
    )
    gva_total_low, gva_total_high = _relative_bounds(
        gva_total_musd,
        _config_float(uncertainty_cfg, ["gva_total_musd"], 0.20),
    )
    household_income_low, household_income_high = _relative_bounds(
        household_income_proxy_musd,
        _config_float(uncertainty_cfg, ["household_income_proxy_musd"], 0.20),
    )

    total_shock_musd = capex_effect_musd + opex_effect_musd
    by_region_records = _build_region_rows(
        summary_diagnostics=summary_diagnostics,
        total_shock_musd=total_shock_musd,
        jobs_direct=jobs_direct,
        jobs_total=jobs_total,
        gva_total_musd=gva_total_musd,
        household_income_proxy_musd=household_income_proxy_musd,
    )
    by_sector_records = _build_sector_rows(
        summary_diagnostics=summary_diagnostics,
        total_shock_musd=total_shock_musd,
        jobs_direct=jobs_direct,
        jobs_total=jobs_total,
    ...
    development = {
        ...
        "inputs": {
            ...
            "operating_shock_total_musd": opex_effect_musd,
            "policy_multiplier": policy_multiplier,
            "levers": lever_values,
        },
        "model_parameters": {
            "fallback_capex_share": fallback_capex_share,
            "fallback_opex_share": fallback_opex_share,
            "jobs_direct_capex_factor": jobs_direct_capex_factor,
            "jobs_direct_opex_factor": jobs_direct_opex_factor,
            "jobs_total_multiplier": jobs_total_multiplier,
            "gva_capex_factor": gva_capex_factor,
            "gva_opex_factor": gva_opex_factor,
            "household_income_share": household_income_share,
        },
        "totals": {
            "jobs_direct": jobs_direct,
            "jobs_total": jobs_total,
        ...
        "uncertainty": {
            ...
            "totals_bounds": {
                "jobs_direct_low": jobs_direct_low,
                "jobs_direct_high": jobs_direct_high,
                "jobs_total_low": jobs_total_low,
                "jobs_total_high": jobs_total_high,
                "gva_total_musd_low": gva_total_low,
                "gva_total_musd_high": gva_total_high,
                "household_income_proxy_musd_low": household_income_low,
                "household_income_proxy_musd_high": household_income_high,
            },
        },
        "by_region": {"records": by_region_records},
    ...
    coupling_manifest = {
        ...
        "fallback_mapping_share": _safe_float(mapping_quality.get("fallback_mapping_share"), 1.0),
        "fallback_mapping_count": int(_safe_float(mapping_quality.get("fallback_mapping_count"), 0.0)),
        "shock_record_count": len(by_sector_records),
        "shock_balance_gap_share": 0.0,
        "warnings_count": len(warnings) + int(_safe_float(mapping_quality.get("mapping_validation_warning_count"), 0.0)),
    ...
    artifacts = {
        "energy_service_balance_csv": f"/api/run/{run_id}/download/exchange/energy_service_balance.csv",
        "development_impacts_json": f"/api/run/{run_id}/development",
...
def _build_mario_development_outputs(
    ...
    exchange_dir, shock_meta, exchange_artifacts, exchange_warnings = _write_exchange_files_for_mario(
        model=model,
        settings=settings,
        req=req,
        run_id=run_id,
    ...
    try:
        surrogate_dev, _, _, _ = _build_surrogate_development_outputs(
            settings=settings,
            summary=summary,
            req=req,
            run_id=run_id,
        ...
        compare_keys = ["jobs_total", "gva_total_musd", "household_income_proxy_musd"]
        deltas: Dict[str, Any] = {}
        for key in compare_keys:
            mario_val = _safe_float(mario_totals.get(key), 0.0)
            surrogate_val = _safe_float(surrogate_totals.get(key), 0.0)
            delta = mario_val - surrogate_val
            pct = (delta / abs(surrogate_val)) if abs(surrogate_val) > 1e-12 else None
            deltas[key] = {
                "mario": mario_val,
        ...
        development.setdefault("diagnostics", {})
        development["diagnostics"]["surrogate_benchmark"] = {"metrics": deltas}
    except Exception:
...
def run_calliope_synchronously(
    settings: Settings,
    req: RunRequest,
    ...
    """
    Runs one Calliope job synchronously.
    ...
    solver_name, solver_warnings = _resolve_solver_for_runtime(settings.solver)
    override_patch = _build_runtime_override_patch(
        settings=settings,
        req=req,
    ...
    _emit_progress(progress_callback, "write_artifacts", 0.78, "Writing CSV artifacts")
    csv_path = run_dir / "results.csv"
    csv_rows = _write_results_csv(csv_path, model)
    if csv_rows <= 0:
        warnings.append("Results CSV export produced no rows.")
    _check_cancel(cancel_requested)

    _emit_progress(progress_callback, "build_summary", 0.86, "Building summary payload")
    summary = build_summary_core(
        ...
        run_profile=run_profile,
        warnings=warnings,
        max_generation_techs=settings.summary_max_generation_techs,
    ...
    if development_warnings:
        summary["warnings"].extend(development_warnings)
    ...
    warnings = list(summary.get("warnings", warnings))

    _write_json(run_dir / "summary.json", summary)
    _write_json(run_dir / "development_impacts.json", development_impacts)
    _write_json(run_dir / "coupling_manifest.json", coupling_manifest)
    _write_json(run_dir / "integrated_results.json", integrated)

    exchange_dir = run_dir / "exchange"
    _ensure_dirs(exchange_dir)
    _write_json(exchange_dir / "development_impacts.json", development_impacts)
    _write_json(exchange_dir / "coupling_manifest.json", coupling_manifest)
    _write_json(exchange_dir / "integrated_results.json", integrated)
