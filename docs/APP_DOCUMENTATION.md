...
cd /Users/ben/Documents/UNDP/SEH/energy-development-modeling
git diff --check
python3 -m py_compile scripts/run_local.py scripts/install_desktop_launcher.py scripts/install_desktop_app.py
cd frontend
npm run build
cd ..
```
...
Use the local runner. It automatically tries later ports:

```bash
scripts/energy-modeling
```

Or specify a starting port:
