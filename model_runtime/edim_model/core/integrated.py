...
def _build_development_indicator_payload(
    ...
    carbon_price = _safe_float(carbon_price_entry.get("value_numeric"), default_carbon_price)
    ...
    return {
        "records": [
            ...
            },
            {
                ...
                "notes": "Filtered UI uses location-level demand and unmet_demand from results.csv, with pool-level values when loc rows are unavailable.",
            },
            {
...
def _build_model_quality(
    ...
    energy_only = str(coupling_manifest.get("development_engine_mode") or "") == "skipped_energy_only" or str(
        coupling_manifest.get("integration_architecture") or ""
    ) == "energy_only"
    ...
    if placeholder_rows > 0 and not energy_only:
        issues.append(
            _quality_issue(
                "placeholder_inputs",
                "error",
    ...
    if mapping_coverage < 0.999 and not energy_only:
    ...
    if mrio_direct_heuristic and not energy_only:
        issues.append(
            _quality_issue(
                "mrio_direct_heuristic",
                "warn",
                "MRIO-direct report assumptions use heuristic effects; bridge-derived values remain authoritative for headline totals.",
            )
        )
        score -= 10.0
    ...
    if mrio_direct_heuristic and not energy_only and status == "production_ready":
        status = "analyst_review"

    if status == "production_ready":
        summary_text = "Production-ready diagnostics: no coupling or consistency issue was detected."
        ...
        summary_text = "Exploratory only: placeholders or consistency gaps materially limit confidence."
...
def validate_integrated_results(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _validate_numeric(confidence.get("unmapped_mapping_share"), "development_confidence.unmapped_mapping_share")
...
def build_integrated_results(
    ...
    confidence = {
        "coupling_mode": str((coupling_manifest or {}).get("development_engine_mode", "mario")),
        "mapping_coverage_share": _safe_float((coupling_manifest or {}).get("mapping_coverage_share"), 0.0),
        "unmapped_mapping_share": _safe_float((coupling_manifest or {}).get("unmapped_mapping_share"), 0.0),
        "warnings_count": int(len(summary.get("warnings") or [])),
        ...
        "overlap_policy": str((coupling_manifest or {}).get("overlap_policy", "")),
    ...
    if isinstance(summary.get("development_indicators"), dict) and summary.get("development_indicators"):
        ...
        development_indicators = _build_development_indicator_payload(
            ...
            default_carbon_price=_safe_float(
...
def create_exchange_bundle_zip(run_dir: Path, artifact_registry: ArtifactRegistry | None = None) -> Path | None:
    run_dir = run_dir.resolve()
    if artifact_registry is not None:
        zip_path = artifact_registry.path_for("exchange_bundle_zip")
        bundle_candidates = artifact_registry.bundle_candidates()
        with ZipFile(zip_path, mode="w", compression=ZIP_DEFLATED) as zf:
            for path in bundle_candidates:
                zf.write(path, arcname=str(path.relative_to(run_dir)))
        artifact_registry.register_existing("exchange_bundle_zip", path=zip_path)
        return zip_path
...
def build_run_report_markdown(summary: Dict[str, Any], integrated: Dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- unmapped_mapping_share: `{_safe_float(confidence.get('unmapped_mapping_share'), 0.0):.6f}`",
            ...
            f"- overlap_policy: `{confidence.get('overlap_policy', '')}`",
            "",
            "## Development Drivers",
            "",
