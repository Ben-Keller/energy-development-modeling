...
HEURISTIC_METHOD = "mrio_direct_heuristic"
                ...
                _annotate_shock_row(
                    {
                        ...
                        "notes": "Signed fossil supply-chain reduction derived from structured fossil delta target. " + common_notes,
                    },
                    record,
                    archetype_id,
                )
                ...
                _annotate_shock_row(
                    {
                        ...
                        "method": HEURISTIC_METHOD,
                        "notes": "Renewable/grid supply-chain gain derived from structured renewable-share target. " + common_notes,
                    },
                    record,
                    archetype_id,
                )
            ...
            _annotate_shock_row(
                {
                    ...
                    "method": HEURISTIC_METHOD,
                    "notes": "Final-demand proxy from structured scenario percentage target. " + common_notes,
                },
                record,
                archetype_id,
            )
    ...
    for scenario_code in ("S1", "S2"):
        ...
        scenarios[scenario_code] = {
            ...
            "provenance": {
                ...
                "generated_from": "inputs/generated/scenario_report_scenarios.json + inputs/scenario_geography_mapping.csv",
            },
        }
    return {
...
def build_integrated_scenario_catalog(config_dir: Path, calliope_root: Path, energy_scenarios: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    africa_mrio = load_or_build_africa_national_mrio_placeholder_scenarios(config_dir, report)
    ...
    target_year_options = [{"value": year, "label": str(year)} for year in sorted(target_years or {2030, 2050})]
    energy_model_engine_options = [
        {"value": "calliope", "label": "Calliope", "runtime_status": "executable"},
    ]
    energy_scenario_options = [
        {
        ...
        for row in energy_scenarios
    ]
    target_scenario_options = [
        {
    ...
    module_configurations = [
        {
            ...
            "scenario_channels": [
                ...
                },
                {
                    ...
                    "required": True,
                    "options": energy_scenario_options,
        ...
        },
        {
            ...
            "scenario_channels": [
                {
                    "module_id": "mrio",
                    "module_kind": "development",
                    "module_label": "MRIO development model",
                    ...
                    "required": True,
                    "options": target_scenario_options,
                ...
                },
                {
                    ...
                    "required": False,
                    "options": shock_mapping_options,
                    "default": shock_mappings[0]["mapping_id"] if shock_mappings else "",
                },
            ],
        },
    ]
    scenario_channels = [
        channel
        for module in module_configurations
        for channel in module.get("scenario_channels", [])
    ]
    return {
    ...
    *,
    ...
    if geography_code == "AFRICA_NATIONAL":
        ...
        return {
            ...
            "notes": (
                "Africa national MRIO scenario applies structured national placeholder records. "
...
def build_scenario_package(
    *,
    ...
    if mrio_scenario_id in AFRICA_MRIO_SCENARIO_LABELS:
        ...
        raise ValueError(f"MRIO scenario '{mrio_scenario_id}' was not found in the structured scenario report.")
    else:
...
def write_scenario_artifacts(
    run_dir: Path,
    scenario_package: Dict[str, Any],
    mrio_direct_inputs: Dict[str, Any] | None = None,
    artifact_registry: ArtifactRegistry | None = None,
) -> None:
    import json

    if artifact_registry is not None:
        artifact_registry.write_json("scenario_package_json", scenario_package, dumps=lambda payload: json.dumps(payload, indent=2))
        artifact_registry.write_json("energy_input_manifest_json", scenario_package.get("energy") or {}, dumps=lambda payload: json.dumps(payload, indent=2))
        artifact_registry.write_json(
            "report_scenario_reference_json",
            (scenario_package.get("mrio_direct") or {}).get("scenario") or {},
            dumps=lambda payload: json.dumps(payload, indent=2),
        )
        artifact_registry.write_json(
            "geography_alignment_json",
            scenario_package.get("geography_alignment") or {},
            dumps=lambda payload: json.dumps(payload, indent=2),
        )
    else:
    ...
    if mrio_direct_inputs is not None:
        ...
        if artifact_registry is not None:
            artifact_registry.write_json("mrio_direct_inputs_json", mrio_direct_inputs, dumps=lambda payload: json.dumps(payload, indent=2))
            artifact_registry.write_csv_rows("mrio_direct_shocks_csv", rows, fieldnames)
        else:
            scenario_dir = run_dir / "scenario"
            scenario_dir.mkdir(parents=True, exist_ok=True)
            (scenario_dir / "mrio_direct_inputs.json").write_text(json.dumps(mrio_direct_inputs, indent=2), encoding="utf-8")
            _write_csv(scenario_dir / "mrio_direct_shocks.csv", rows, fieldnames)
